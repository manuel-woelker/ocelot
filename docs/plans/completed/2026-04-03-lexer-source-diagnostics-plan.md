...
- [x] Change `lex()` to accept `&SourceFile` and `&mut CompilationContext` instead of `&str`.
- [x] Implement unterminated-string `SourceDiagnostic` emission in the lexer.
- [x] Add colocated lexer tests for diagnostic contents, spans, and excerpts.
- [x] Thread the shared `CompilationContext` into `Parser::new()`.
- [x] Update parser-facing APIs so diagnostics live in the shared compilation context instead of plain `OcelotError` strings.
- [x] Add parser tests proving lexer failures surface as structured diagnostics.
- [x] Run `nao check`.
