...
This slice landed first-class file-local function imports:

- the lexer now reserves `use`
- the AST now includes [`UseItem`](/data/projects/ocelot/crates/ast/src/use_item.rs) for top-level import declarations
- the parser now accepts both `use helper::greet;` and grouped imports such as `use helper::{greet, wave};`
- [`ProgramEnvironment`](/data/projects/ocelot/crates/ast/src/program_environment.rs) now tracks file-local imported function bindings keyed by source file
- the resolver now registers imports after function registration, validates imported module/function names, rejects duplicate imports and local-function conflicts, and resolves imported names in both top-level statements and function bodies
- the engine now runs import registration as part of multi-file program compilation
- [`25.02 Modules - Imports`](/data/projects/ocelot/docs/spec/25.02%20Modules%20-%20Imports.md) now documents the feature with successful and failing examples
- parser, resolver, engine, and spec-validation tests were extended for imports
- `cargo test -p ocelot-parser -p ocelot-ast -p ocelot-resolver -p ocelot-engine -p ocelot-spec-validation`
- `nao check`

# What concrete tasks should track this plan?

- [x] Add a spec chapter documenting `use` imports and grouped imports.
- [x] Reserve `use` in the lexer.
- [x] Add AST support for top-level `use` items.
- [x] Parse single-name and grouped imports.
- [x] Introduce resolver state for file-local imported function bindings.
- [x] Register and validate imports before resolving statements and function bodies.
- [x] Update unqualified function resolution to consult imports between local functions and builtins.
- [x] Add resolver diagnostics for unknown modules, unknown imported functions, and duplicate/conflicting imports.
- [x] Add or update parser, resolver, engine, and spec-validation tests.
- [x] Run `nao check`.
