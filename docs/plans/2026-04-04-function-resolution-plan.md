...
- change `resolve(&Script)` to `resolve(&mut Script, environment: &ProgramEnvironment)` or an equivalent API shape
...
- [ ] Add a `ProgramEnvironment` that owns the native function definition table and symbol table shared by resolution and interpretation.
- [ ] Add a `FunctionIndex` newtype and use it in the function symbol table, resolved call metadata, and interpreter dispatch helpers.
- [ ] Seed `ProgramEnvironment` with `println`, `assert`, and `assert_eq`.
- [ ] Extend `CallExpression` to store an optional resolved function table index.
- [ ] Add a `CallExpression` helper returning `OcelotResult<FunctionIndex>` for resolved-index access.
- [ ] Implement resolver traversal that resolves call expressions across scripts and test bodies.
- [ ] Add resolver diagnostics and tests for unknown function names and correct callee spans.
- [ ] Run the resolver from the engine after parsing and before interpretation.
- [ ] Update the interpreter to dispatch calls by resolved function index instead of callee string matching.
- [ ] Update interpreter and engine tests for the new phase boundary and dispatch model.
- [ ] Run `nao check`.
