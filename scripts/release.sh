...
RELEASE_BRANCH="${OCELOT_RELEASE_BRANCH:-main}"
WAIT_ATTEMPTS="${OCELOT_RELEASE_WAIT_ATTEMPTS:-30}"
WAIT_SECONDS="${OCELOT_RELEASE_WAIT_SECONDS:-10}"
DRY_RUN=0
MODE="publish"
TARGET_VERSION=""
...
main() {
  ...
  local version
  version="${TARGET_VERSION:-$(release_version)}"
  local tag_name="v$version"

  run_pre_release_checks "$version" "$tag_name"

  if [[ "$DRY_RUN" -eq 1 ]]; then
    log "dry run complete; skipping publish and tag push"
    return
  fi

  publish_crates "$version"
  create_and_push_tag "$tag_name"
  log "release $tag_name complete"
}
